...
col.1 <- adjustcolor("orange red", alpha=0.6)
col.2 <- adjustcolor("orange", alpha=0.6)
...
### Landings average 2014-2016 by country  {data-height=850}
...
```{r,eval=T}
library(rasterVis)
pp<-stack("./data/envprod")
gplot(pp)+geom_tile(aes(fill=value))+
	scale_fill_gradient(low="blue",high="green",trans="log10")+
	facet_wrap(~variable)+#raster(aes(fill="Prod_2000"))
                theme_bw()+
                xlab("Longitude")+ylab("Latitude")
...
```{r,eval=T}
sst<-stack("./data/envtemp")
gplot(sst)+geom_tile(aes(fill=value))+
